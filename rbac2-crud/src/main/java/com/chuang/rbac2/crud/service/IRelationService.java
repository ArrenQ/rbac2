...
import com.chuang.rbac2.crud.entity.Relation;
import com.chuang.rbac2.crud.enums.RelationType;
 ...
 * @author chuang
 * @since 2020-12-20
 * @deprecated 这种关系结构虽然灵活，快速，且没有层级限制。但实现节点排序比较困难。在项目中关系结构使用 id, parentId, parents 三个字段来做逻辑更简单（虽然parents长度可能会越界，但在本项目中还不太可能出现）
 */
public interface IRelationService extends IRowQueryService<Relation> {
    ...
    default void moveAndDeleteChildren(Integer from, Integer to, RelationType type, @Nullable String condition1, @Nullable String condition2) {
        ...
        // 检查 to 是不是 from 的子节点
        long count = count(new QueryWrapper<>(new Relation()
                .setAncestor(from)
                .setDescendant(to)
                .setType(type)
